...
export const Home: React.FC = () => {
    ...
    return (
        <div className="flex h-full w-full bg-surface items-center justify-center">
            {/* Left: Custom Mode */}
            <div className="flex-1 flex flex-col items-center justify-center p-8 border-r border-white/10 h-full relative overflow-hidden group bg-transparent">
                <div className="absolute inset-0 bg-gradient-to-br from-app to-surface opacity-50 group-hover:opacity-30 transition-opacity"></div>
                    ...
                    <div className="grid grid-cols-1 gap-4 w-full">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <button
                                ...
                                className="flex flex-col items-center justify-center gap-2 py-4 bg-surface hover:bg-white/10 border border-white/10 hover:border-primary/50 rounded-xl text-text-sub hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                <Swords className={`w-6 h-6 ${isConnecting ? 'animate-pulse' : ''}`} />
                                <span className="text-xs font-medium">
                                    {isConnecting ? '서버 연결 중...' : '멀티플레이'}
                                </span>
                            </button>
                            <button disabled className="flex flex-col items-center justify-center gap-2 py-4 bg-black/20 border border-white/5 rounded-xl text-text-sub/50 cursor-not-allowed">
